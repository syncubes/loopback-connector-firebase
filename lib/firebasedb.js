...
FirebaseDB.prototype.all = function all(model, filter, options, callback) {
      ...
      if (id && typeof id !== 'object') {
        this.find(model, id, function(err, data){
          if (filter && filter.include) {
            self._models[model].model.include([data], filter.include, options, callback);
          } else {
            callback(err,[data]);
          }
        });
  ...
  newRef.once('value', function(snapshot){
    ...
    if (values) {
      ...
      if (filter && filter.include) {
        self._models[model].model.include(results, filter.include, options, callback);
      } else {
...
FirebaseDB.prototype.destroyAll = function destroyAll(model, data, callback) {
  ...
  var self = this;
  if (data && data.id) {
    var id = data.id.inq && data.id.inq.length ? data.id.inq[0] : data.id;
    return this.destroy(model, id, callback);
  } else if (callback){
 ...
 * Replace model by Id
 */
FirebaseDB.prototype.replaceById = function replaceById(model, id, data, callback) {
  console.log("replaceById()");

  if (id) {
    var ref = client.child(model + '/' + id);
    ref.set(JSON.parse(JSON.stringify(data)), function(err){callback(err, data.id);});
  }
